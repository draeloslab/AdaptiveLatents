...
class proSVD:
    ...
    def initialize(self, A_init, Q_init=None, B_init=None):
        ...
        if self.centering:
            self.input_center = np.mean(A_init, axis=1)
            A_init -= self.input_center
    ...
    def run_on(self, X):
        ...
        for i in np.arange(X.shape[1]):
            self.updateSVD(X[:, i:i+1])

            X_reduced[:,i:i+1] = self.Q.T @ X[:, i:i+1]
        return X_reduced
    ...
    def updateSVD(self, A, ref_basis=None):
        if self.centering:
            self.input_center += (A - self.input_center).sum(axis=1)/(self.n_columns_seen + A.shape[1])
